...
def generate_plots():
    ...
    convergence_curves = joblib.load('results_improved/convergence_curves.pkl')
    ...
    df_results[['model_name', 'mae', 'rmse', 'r2']].to_csv('results_improved/performance_comparison.csv', index=False)
    ...
    for metric in metrics:
        ...
        plt.title(f'Improved Model Comparison - {metric.upper()}')
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.savefig(f'plots_improved/{metric}_comparison.png')
        plt.close()
    ...
    plt.title('Convergence Curves (Improved Data)')
    plt.xlabel('Epoch')
    plt.ylabel('Fitness (MSE on Log Scale)')
    plt.legend()
    plt.tight_layout()
    plt.savefig('plots_improved/convergence_curves.png')
    plt.close()
    
    # 4. Feature Importance (using the best MLP model)
    ...
    plt.title("Feature Importance (Improved MLP Model)")
    plt.tight_layout()
    plt.savefig('plots_improved/feature_importance.png')
    plt.close()
    
    print("Improved plots generated in plots_improved/ directory.")
